...
Phase: Plan written, not yet executed
Done: spec approved (commit 148e21e); all plan-time probes verified (see Data Contracts)
Next: Task 1 (prerequisite gate + scaffold)
Blocked: `python3-pyside6` not installed — Task 1 Step 1 gates on Dustin running the install command
...
def test_parse_list_timers_tolerates_null_next_and_last():
    text = '[{"next":null,"left":null,"last":null,"passed":null,"unit":"x.timer","activates":"x.service"}]'
...
def test_parse_list_units_fields():
    ...
    assert {r.active for r in rows} <= {"active", "inactive", "failed", "activating", "deactivating", "reloading"}
...
class SystemdClient(QObject):
    ...
    def list_services(self, scope: str) -> bool:
        return self.request(
            f"services:{scope}",
            [self._systemctl, *self._scope_args(scope), "list-units", "--type=service", "-o", "json"],
        )
    ...
    def fetch_log(self, scope: str, unit: str) -> bool:
        return self.request(
            f"log:{scope}:{unit}",
            [self._journalctl, *self._scope_args(scope), "-u", unit, "-o", "json", "-n", "200", "--no-pager"],
        )
...
def action_argv(verb: str, scope: str, unit: str) -> list[str]:
    ...
    if verb not in ALLOWED_VERBS:
        raise ActionNotAllowed(f"verb {verb!r} not allowed in v1 (allowed: {sorted(ALLOWED_VERBS)})")
...
TIMERS = [
    TimerRow("a.timer", "a.service", usec(datetime(2026, 6, 10, 23, 10)), usec(datetime(2026, 6, 10, 17, 11)), ),
...
class MainWindow(QMainWindow):
    ...
    def _fill_tab(self, kind: str, stdout: str) -> None:
        if kind == "log":
            ...
            cal_lines = [
                line.strip() for line in stdout.splitlines() if line.strip().startswith("OnCalendar=")
            ]
            if cal_lines:
    ...
    def _update_action_enablement(self) -> None:
        ...
        for act in self.action_buttons:
            ...
            act.setToolTip("" if self.scope == SCOPE_USER else "system units are read-only by design")
