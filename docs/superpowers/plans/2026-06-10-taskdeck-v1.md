...
## Status (updated 2026-06-11, QA fix batch shipped)
Phase: Task 9 nearly closed — three-phase QA COMPLETE (artifacts in docs/qa/2026-06-11-v1/: 4×Phase A, 4×Phase B, Phase C synthesis), Phase C fix batch shipped (2fea72e). Manual launch found USER-P1 (Run-now false timeout) — fixed. Dustin's verdict on the FIXED build + the cadence column (Task 10) remain.
Done: T1-T8 + QA fix batch. Suite: 89 hermetic + 2 live, ruff+mypy clean. Open deferments: DEF-T4-01 (QProcess leak, segfault-blocked), DEF-A-01 (LOW), DEF-V11-01 (stderr plumbing), DEF-V11-02 (schedule source → Task 10), DEF-V11-03 (pane header), KNOWN-V1 (bare enable/disable).
Next: Task 10 — cadence column (Dustin-requested 2026-06-11) + DEF-V11-02 schedule-source redesign, one batched show feeds both.
Blocked: nothing
...
- **DEF-V11-02** (MEDIUM, QA AT-F6 with all-hands agreement on direction,
  target: Task 10): Schedule-tab source redesign — replace the cat→extract→
  analyze text chain (wrong under drop-in overrides, `OnCalendar =` spacing,
  multi-line schedules) with batched
  `show -p TimersCalendar,TimersMonotonic,NextElapseUSecRealtime`, which is
  ALSO the data source the cadence column needs — one fetch serves both, and
  the chain deletion removes the least-tested orchestration in the window.
  Fail-loud parsing contract for the structured TimersCalendar value (SFH).
  Calendar-chain pin tests deliberately NOT written pre-redesign (TA).
