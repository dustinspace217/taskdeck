...
- **DEF-T4-01** (MEDIUM, found by Task 4 spec+quality reviewers, deferred at commit
  ...
  Fix direction for a future session: (1) check PySide6 releases past 6.11.1 for
  deferred-delete fixes and retry (a) then (b); (2) if unfixed, try
  `proc.setParent(None)` after terminal handling to hand ownership to Python's GC
  (synchronous destruction, avoids the DeferredDelete event entirely); (3) last
  resort: per-request_id QProcess reuse pool (bounded by ~10 ids).
