...
## Status (updated 2026-06-12, Task 10 + QA fix batch shipped)
Phase: Task 10 COMPLETE — cadence column + show-based Schedule tab shipped (39c145e), three-phase QA run (code-reviewer + test-analyzer, cross-examined; artifacts in docs/qa/2026-06-12-task10/), QA fix batch shipped. Remaining: privacy audit + public GitHub push, then Dustin's verdict on the build.
Done: T1-T8 + Task 10 + both QA fix batches. Suite: 116 hermetic + 3 live, ruff+mypy clean. DEF-V11-02 RESOLVED (Task 10 — Schedule tab now reads `systemctl show` effective triggers, not cat-scraped unit-file text). Open deferments: DEF-T4-01 (QProcess leak, segfault-blocked), DEF-A-01 (LOW), DEF-V11-01 (stderr plumbing), DEF-V11-03 (pane header), DEF-T10-01 (multi-calendar Schedule tab untested, LOW), KNOWN-V1 (bare enable/disable).
Next: privacy audit (fixtures/docs — clean so far: machine-id zeroed, no creds/location/email) → create public repo under dustinspace217 → push v1-implementation.
Blocked: nothing

### Task 10 deviation summary (2026-06-12)
What changed from plan-as-written: Task 10 was sketched in the Status block as "cadence column + DEF-V11-02 redesign, one batched show feeds both." Built exactly that, with one structural refinement found during implementation: the Cadence column and the Schedule tab use SEPARATE show fetches (a batched `fetch_schedules` for the table's cadence column across all timers, and a per-selection `fetch_tab_schedule` for the detail tab) rather than one shared show. Reason: the table needs all timers at once (batch), the tab needs one unit on selection (single) with a chained systemd-analyze elapse preview — different cardinalities, so one query couldn't serve both without waste. Classification: behavioral-change (additive; no prior behavior altered).
QA fix batch deviations: two P1 bugs found by review (CADENCE-1 classifier misbuckets, FREEZE-1 parse-raise freezes both views) — both behavioral fixes, not deferments. All other findings fixed in-batch. One new deferment registered (DEF-T10-01, below).
...
- [x] **Step 1: Gate on PySide6.** Run: `rpm -q python3-pyside6 python3-pytest-qt`
...
- [x] **Step 2: Write `pyproject.toml`** (tool config only — no build backend; the app
...
- [x] **Step 3: Create the package and test scaffolding.**
...
- [x] **Step 6: Amend the spec's layout line.** In
...
- [x] **Step 7: Verify lint passes on the empty scaffold.**
...
- [x] **Step 1: Capture real fixtures from this machine** (deterministic forever after;
...
- [x] **Step 3: Implement** (start of `taskdeck/models.py`):
...
- [x] **Step 4: Run tests.** `python3 -m pytest tests/test_format.py -v` — all PASS.
...
- [x] **Step 2: Run to verify failure.** `python3 -m pytest tests/test_actions.py -v` — ImportError.

- [x] **Step 3: Implement `taskdeck/actions.py`:**
...
- [x] **Step 4: Run tests.** `python3 -m pytest tests/test_actions.py -v` — all PASS.
...
- [x] **Step 2: Run to verify failure.** `python3 -m pytest tests/test_models.py -v` — ImportError.

- [x] **Step 3: Implement the model** (append to `taskdeck/models.py`):
...
- [x] **Step 4: Run tests.** `python3 -m pytest tests/test_models.py -v` — all PASS.
...
- [x] **Step 2: Run to verify failure.** `python3 -m pytest tests/test_smoke.py -v` — ImportError.

- [x] **Step 3: Implement `taskdeck/main_window.py`:**
...
- [x] **Step 4: Run the smoke test.** `python3 -m pytest tests/test_smoke.py -v` — PASS,
...
**Task 7 review checklist (handoffs from the Task 4 quality review):**
- `_on_finished` request-id dispatch MUST use `split(":", 2)` — `:` is legal in unit
  names, so `log:user:foo:bar.service` has colons inside the unit part.
- Action buttons: `run_action` coalesces by `action:<unit>` (verb NOT in the id). A
  start-then-stop on the same unit while the first is in flight returns False — the
  UI must surface that rejection in the status bar, never drop it silently.
- `fetch_results(scope, [])` would dump manager properties and break the parser —
  the planned `if self._result_units:` guard is load-bearing; reviewer must confirm it.
- (From Task 6 review) `self.proxy.setSortRole(ROLE_SORT)` must be present — without
  it the Next/Last columns sort alphabetically by display string.
...
- [x] **Step 1: Write `taskdeck/app.py`:**
...
- [x] **Step 2: Write the opt-in integration tests:**
...
## Deferments (review findings knowingly deferred)

Full QA disposition record: docs/qa/2026-06-11-v1/phaseC-synthesis.md.

- **DEF-V11-01** (MEDIUM, QA Phase C consolidation of SFH-F4 journalctl half +
  AT-F5 + old DEF-T2-01, target: v1.1): full stderr-on-success plumbing for the
  journal path. journalctl exits 0 while printing corruption warnings and the
  "not seeing system messages" permission hint to stderr — both currently
  discarded, so a partial or permission-limited log renders as confidently
  complete. Staged design (Phase B): widen the finished signal (or parallel
  channel); journalctl first (no informational noise there); verbatim
  secondary text, NO "warning" framing (enable/disable print "Created
  symlink…" to stderr on success — alarm-fatigue trap); a fakebin
  exit-0+stderr stub test is mandatory (path unreachable on this wheel
  account). The action-kind half already shipped in the v1 batch (narrow
  passthrough, commit 2fea72e).

- **DEF-V11-02 — RESOLVED 2026-06-12 (Task 10, commit 39c145e + QA fix batch)**
  (was MEDIUM, QA AT-F6 with all-hands agreement on direction): Schedule-tab
  source redesign — replaced the cat→extract→analyze text chain (wrong under
  drop-in overrides, `OnCalendar =` spacing, multi-line schedules) with
  `show -p TimersCalendar,TimersMonotonic,NextElapseUSecRealtime`. The Schedule
  tab's `fetch_tab_schedule` (per-selection) and the cadence column's
  `fetch_schedules` (batched) both read it; the cat branch now serves only the
  Unit-file tab. Fail-loud parsing contract shipped (`parse_show_schedules`
  raises on unrecognized trigger lines). Calendar-chain pin tests written in
  Task 10; QA fix batch added the freeze-prevention + barrier + seam pins.

- **DEF-T10-01** (LOW, QA 2026-06-12 head-agent deferment, commit 39c145e /
  QA-fix-batch, defer target: indefinite): the multi-CALENDAR-trigger Schedule
  tab has no test. A timer with two `OnCalendar=` lines renders both, but only
  `info.calendar[0]` chains to systemd-analyze for the elapse preview — correct
  by construction (list rendering + an explicit `[0]` index), so pinning it is
  nice-to-have, not load-bearing. No fixture block currently carries two
  calendar triggers either. Fix direction: add a two-calendar fixture block and
  a schedtab test asserting both lines render and exactly one fetch_calendar
  fires (for calendar[0]). Obsolescence: irrelevant if the elapse preview is
  ever removed or made multi-trigger.

- **DEF-V11-03** (LOW, QA AT-F12/SFH endorsement, target: v1.1+): pane-header
  label ("unit — fetched HH:MM:SS") making any residual tab staleness
  self-evident structurally.

- **KNOWN-V1**: enable/disable run WITHOUT --now (spec'd bare verbs): Enable
  doesn't start a unit, Disable doesn't stop a running one. Revisit alongside
  DEF-V11-01 (the no-[Install] explanation now surfaces via the action-stderr
  passthrough; probed exit 0).

- **NOTE-T7** (residual informational): (a) act_stop tests must monkeypatch
  QMessageBox.question (modal blocks under offscreen QPA); (b) view switching
  does a refresh round-trip instead of a local re-render — cheap UX win later.
  [(c) resolved: _data_scope gate, commit 2fea72e. The (b) failure-path risk
  was re-graded and fixed via the same gate.]

- **RESOLVED 2026-06-11 (commit 2fea72e, QA Phase C batch):** NOTE-T3 →
  LC_TIME=C in conftest. DEF-T2-01 → folded into DEF-V11-01. NOTE-T7(a)'s
  practical consequence (zero action-wiring coverage) → action test battery.
  USER-P1 (Run-now false timeout) → --no-block. TA-F1+AT-F1 compound lie →
  ts_missing + never-ran gate. Status washout → permanent freshness label +
  kind-aware clearing. Tab staleness family + Race B → dedup invalidation
  battery. Cross-scope results race → by-request-id alignment. Catch gap →
  field validation + render guards + excepthook (probed HOOK_FIRED).
  Scroll yank → capture/restore. Show-inactive toggle → implemented.

- **DEF-T2-01 — RESOLVED-INTO DEF-V11-01** (was: parse_journal discards the
  corrupt-line skip count; same missing-channel root as the stderr work).

- **DEF-A-01** (LOW, found by code-reviewer + adversarial-tester independently,
  commit 233199c, defer target: Task 9 close-out or indefinite):
  `request("id", [])` passes an empty `argv` to `QProcess.start(argv[0], argv[1:])`,
  raising `IndexError` immediately instead of emitting `failed`. Breaks the signal
  contract for direct `request()` misuse. All typed conveniences (list_timers, etc.)
  supply valid argv, so this only fires on caller error, not app usage.
  Fix direction: add `if not argv: self.failed.emit(request_id, "empty argv"); return True`
  at the top of `request()`. One line, low risk.
  Obsolescence: if typed conveniences are the ONLY callers (enforced by making `request`
  private), this is irrelevant.

- **DEF-A-02 — RESOLVED at c323ce8**: `test_spawn_failure_emits_failed` added during
  the Task 4 quality-review fix pass (exercises errorOccurred(FailedToStart) with no
  finished signal).

- **DEF-T4-01** (MEDIUM, found by Task 4 spec+quality reviewers, deferred at commit
  c323ce8, defer target: indefinite / PySide6 update):
  Dead QProcess objects are never freed — they accumulate as QObject children of
  SystemdClient for its lifetime (~3 per 10s refresh cycle ≈ 26k/day if the window
  stays open; tens of MB/day). Power-of-Ten rule 3 violation, knowingly accepted.
  WHY DEFERRED — both standard fixes SEGFAULT, probed empirically 2026-06-11 on
  PySide6 6.11.1 / Python 3.14.5: (a) `proc.deleteLater()` inside our Python slot →
  segfault in pytest-qt's `_process_events` on the first test; (b) the canonical
  `proc.finished.connect(proc.deleteLater)` idiom → same segfault. Crash is in the
  deferred-deletion event dispatch (native frames in libpython/shiboken) — smells
  like an upstream PySide6-on-3.14 bug.
  Fix direction for a future session (probe in this order; (0) and (4) added by the
  Task 4 reviewer as hypotheses, unverified): (0) CHEAPEST — explicitly disconnect()
  all three signal connections + stop the watchdog in the terminal handler BEFORE
  deleteLater(); if the crash is shiboken releasing live Python-slot connections
  during deferred destruction (consistent with the observed frames), plain
  deleteLater may then survive. (1) Check PySide6 releases past 6.11.1 for
  deferred-delete fixes and retry both original shapes. (2) `proc.setParent(None)`
  + Python GC — CAVEAT from reviewer: likely a silent no-op leak, because the C++
  signal manager holds the closure refs across the shiboken boundary where Python's
  cycle collector can't traverse; if attempted, pair with explicit disconnects and
  verify with an RSS soak. (3) Per-request_id QProcess reuse pool (bounded ~10 ids).
  (4) `QTimer.singleShot(0, lambda: shiboken6.delete(proc))` — synchronous C++
  destruction from a fresh event-loop turn, bypassing the DeferredDelete machinery
  where both observed crashes live.
  Mitigation meanwhile: the app is an on-demand window (spec decision), so exposure
  is session-length-bounded; closing the window frees everything.
  Obsolescence: PySide6 fix upstream, or v2 D-Bus data layer.
  SIZING UPDATES (QA 2026-06-11): the timeout path never drains stdout, so a
  repeatedly-timing-out journalctl pins its buffered output — megabytes per
  kill, not kilobytes (AT); per-selection fetches add 3-4 procs per row click
  on top of the ~3/10s refresh estimate (CR).
