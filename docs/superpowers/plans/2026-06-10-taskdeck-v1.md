...
## Status (updated 2026-06-11, Task 8 code approved; manual launch pending Dustin)
Phase: Executing, Tasks 1-8 of 9 complete (T8 Step 4 = Dustin's hands-on launch verdict, app currently RUNNING on his desktop via `python3 -m taskdeck`, launched by controller)
Done: T1-T7 (git log; open deferments: DEF-T2-01, DEF-T4-01, DEF-A-01); T8 entry point + live-contract tests (0352a1b+c2e6962: int(exec) cast, __main__ guard, readable integration failures; realsystemd 2/2 against live instance; offscreen 5s launch probe clean). Hermetic suite: 56 passed. Gates clean (7 source files).
Next: Task 9 close-out — three-phase QA review (Phase A running), deviation summary, final status
Blocked: nothing (Dustin's launch verdict is the only outstanding external input)
...
- **NOTE-T7** (informational, from the Task 7 review — no action for v1): (a) any
  future test touching act_stop must monkeypatch QMessageBox.question first — it's a
  modal exec() that blocks forever under offscreen QPA; (b) view switching does a full
  refresh round-trip instead of re-rendering cached lists — cheap UX win later;
  (c) programmatic set_scope() doesn't sync scope_box's displayed index (production
  always goes through the combo, divergence is test-only).

- **NOTE-T3** (informational, no action): test_format.py asserts English %a/%b names.
  Deterministic today because Python leaves LC_TIME at "C" unless setlocale is called;
  Qt adopts the system locale at QApplication construction (documented behavior,
  unprobed here), so on a non-English machine the in-app strings localize and — if a
  qtbot test ever runs before test_format in-process — these assertions could fail.
  Fix if it ever fires: force LC_TIME=C in tests/conftest.py.

- **DEF-T2-01** (LOW, found by Task 2 quality reviewer, deferred at commit 0a649db,
  target: Task 7 log-tab wiring or v2): `parse_journal` silently discards the count of
  skipped corrupt lines when at least one line parses (19-of-20-corrupt shows a 1-entry
  log with no hint). Fix direction: return or log the skip count and surface it in the
  Log tab ("N corrupt lines skipped"). Obsolete if: log fetching moves to journald's
  native API (v2+).
