...
## Status (updated 2026-06-11, Task 4 complete)
Phase: Executing (subagent-driven), Tasks 1-4 of 9 complete
Done: T1 scaffold; T2 parsers+fixtures (14 tests, DEF-T2-01); T3 time rendering (24 tests); T4 SystemdClient QProcess async (28 tests, commits 233199c+cf8c290, DEF-A-01+DEF-A-02)
...
- **DEF-A-02** (LOW, found by test-analyzer + adversarial-tester, commit 233199c,
  defer target: Task 9 close-out):
  The `on_error` code path (QProcess spawn failure — binary not found, permission denied)
  is untested. `fake_fail` tests non-zero exit (different path); no test covers
  `proc.errorOccurred` firing without `proc.finished`.
  Fix direction: add `test_spawn_failure_emits_failed(qtbot)` using a nonexistent binary
  path (e.g., `FAKEBIN / "does_not_exist"`). One test, five lines.
  Obsolescence: irrelevant if SystemdClient is replaced with D-Bus (v2+).
