...
- **WATCH-1 (Task 3, latent, unreachable as specified):** `_collapse` in `calendar_model.py` keys its `idx` map by slot *value*, so duplicate `when` values in `slots` would degenerate the region-merge. Unreachable in v1 — single-timer slots come from one `parse_projection` (strictly increasing, distinct) and multi-trigger timers resolve to the *smallest interval*, never a concatenated slot list. If a future change ever merges multiple expressions into one slot list, switch `_collapse` to a keyless positional walk over the sorted missed list. (Flagged by the Phase-1 adversarial verifier; left unfixed per smallest-sufficient — don't harden an unreachable path.)
