...
## Watch-items (found during execution, not bugs as specified)
- **WATCH-1 (Task 3) — RESOLVED by QA fix F5 (2026-06-20):** `_collapse` in `calendar_model.py` keys its `idx` map by slot *value*, so duplicate `when` values in `slots` degenerate the region-merge. This was flagged "unreachable as specified," but QA's adversarial-tester upgraded it to LIVE: fix F4 (multi-trigger) now unions several expressions' slots into one concatenated list, and a DST fall-back makes systemd-analyze emit the same instant twice — both produce duplicate `when`s. Fixed in `compute_gaps` by deduping with `sorted(set(slots))` before the walk (the single guard; `_collapse` stays a simple value-keyed positional walk, with its docstring now stating the dedup precondition). Covered by `test_collapse_tolerates_duplicate_slots`, `test_dst_fall_back_duplicate_instants_do_not_break_collapse`, and `test_collapse_split_case_ran_slot_between_two_missed`.
